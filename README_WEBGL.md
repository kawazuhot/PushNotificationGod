...
## iPhone Safari向けメモリ対策

iPhone SafariではWebGLメモリがPCブラウザより厳しく、メモリ不足時に `RuntimeError: Out of bounds memory access` が出ることがあります。
このMVPでは以下の対策を入れています。

- WebGL Initial Memory Sizeを `128 MB` に設定
- WebGL Maximum Memory Sizeを `512 MB` に設定
- GitHub Pages向けにDecompression FallbackをON
- Data CachingはOFF
- 日本語フォントは `HiraginoKakuGothicW6.ttc` 1本に削減
- 通知カードは `maxVisibleBeforeSpawn = 6` で追加上限を設定
- WebGLビルド出力は毎回クリーンして、古い `WebGL.data 2.unityweb` などを残さない

公開前は `docs/Build` に過去ビルドの番号付きファイルが残っていないことを確認してください。
