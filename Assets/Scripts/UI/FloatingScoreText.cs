...
namespace PushNotificationGod.UI
{
    public class FloatingScoreText : MonoBehaviour
    {
        ...
        [SerializeField] private float riseDistance = 68f;
        ...
        private System.Collections.IEnumerator Animate()
        {
            ...
            while (elapsed < duration)
            {
                ...
                canvasGroup.alpha = t < 0.55f ? 1f : Mathf.Lerp(1f, 0f, (t - 0.55f) / 0.45f);
                yield return null;
            }
        ...
        private static float ScoreScale(float t)
        {
            if (t < 0.18f)
            {
                return Mathf.Lerp(0.8f, 1.25f, t / 0.18f);
            }

            if (t < 0.34f)
            {
                return Mathf.Lerp(1.25f, 1.05f, (t - 0.18f) / 0.16f);
            }

            return Mathf.Lerp(1.05f, 0.98f, (t - 0.34f) / 0.66f);
        }
    }
}
